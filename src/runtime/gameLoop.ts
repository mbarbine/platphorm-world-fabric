...
function tick() {
  ...
  // Keep last 3 seconds (90 ticks).
  // Optimization: Map keys maintain insertion order (monotonically increasing ticks).
  // Using .keys().next().value retrieves the oldest tick in O(1) time with 0 array allocations,
  // avoiding O(N) array spread and Math.min iterations every tick.
  if (stateHistory.size > 90) {
  ...
  // Pre-calculate squared interest radius to avoid redundant multiplication inside the filtering loop.
