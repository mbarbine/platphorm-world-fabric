...
function tick() {
  ...
  if (stateHistory.size > 90) { // Keep last 3 seconds
    // Map entries maintain insertion order; the first key is the oldest/minimum tick.
    // Avoids array allocation and O(N) Math.min argument spreading on every 30Hz tick.
