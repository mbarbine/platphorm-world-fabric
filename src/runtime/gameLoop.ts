...
function tick() {
  ...
  if (stateHistory.size > 90) { // Keep last 3 seconds
    // Map iterates in insertion order, so the first key is always the oldest tick.
    // Avoids array allocation and Math.min spread overhead on every tick.
  ...
  // Pre-calculate squared interest radius to avoid repeated multiplication in the filter loop
